...
async function runTests() {
  ...
  assert('Lead language is de', addRes.body.lead?.language === 'de');
  ...
  assert('POST /api/add-lead (English) returns 201', addEnRes.status === 201);
  assert('English lead language is en', addEnRes.body.lead?.language === 'en');
  ...
  assert('Search by name works', searchRes.body.length === 1 && searchRes.body[0].name === 'Hans Müller');

  const searchHotel = await req('GET', '/api/leads?search=Grand', null, token);
  assert('Search by hotel works', searchHotel.body.length === 1);
  ...
  const lead3 = await req('POST', '/api/add-lead', {
    name: 'Maria Schmidt', email: 'maria@hotel.de',
    hotel: 'Stadthotel', location: 'Hamburg', language: 'de',
  }, token);
  ...
  const lead4 = await req('POST', '/api/add-lead', {
    name: 'Klaus Weber', email: 'klaus@hotel.de',
    hotel: 'Berghotel', location: 'Frankfurt', language: 'de',
  }, token);
  ...
  const visitRes = await req('POST', '/track/visit', {
    trackingId,
    page: 'https://boorgen.com/hotel-analysis',
...
(async () => {
  console.log('═══════════════════════════════════════════════');
  console.log('  BOORGEN CRM — End-to-End Test Suite');
  console.log('═══════════════════════════════════════════════');
