...
(async () => {
  console.log('\n🔥 BOORGEN Live Smoke Test (port 3000)\n');
  ...
  const add = await req('POST', '/api/add-lead', {
    name: 'Smoke Test DE', email: `smoke_de_${ts}@test.com`,
    hotel: 'Testhotel', location: 'Berlin', language: 'de',
  }, token);
  check('Add DE lead (201)', add.status === 201, `status=${add.status}`);
  ...
  const addEn = await req('POST', '/api/add-lead', {
    name: 'Smoke Test EN', email: `smoke_en_${ts}@test.com`,
    hotel: 'Test Hotel', location: 'Munich', language: 'en',
  }, token);
  check('Add EN lead (201)', addEn.status === 201, `status=${addEn.status}`);
  ...
  // Search
  const search = await req('GET', `/api/leads?search=Smoke+Test+DE`, null, token);
  check('Search by name works', search.body.some(l => l.email === `smoke_de_${ts}@test.com`));
  ...
  const fresh = await req('GET', `/api/leads?search=smoke_de_${ts}`, null, token);
